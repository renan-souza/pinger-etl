 ...
 *
 */
public class FileHandlerHourly  {
	...
	public FileHandlerHourly(String transformedFilesDirectory, String tick, String metric, String year, String month) {
		this.year = year;
	...
	private synchronized void checkHeap() {
		if (fileContent.length() >= CUT_HEAP_MEMORY) {
			try {
				...
				Logger.error(FileHandlerHourly.class.getName()+".checkHeap " , e);
			}
		}
	}
	...
	public void writeContentAndClean() {
		try {
			...
			Logger.error(FileHandlerHourly.class.getName()+".writeContentAndClean " ,  e);
		}
	}
	
	private String getFileName() {
		String fileName = Utils.getFileNameBeginningHourly(tick, metric, year, month) + "_" + currentFileIndex + ".csv";
		return fileName;
	}
