			...
			try {
				...
				for (int j = 0; j < nodeLineInfo.length; j++){
					...
					case 0:
						...
						nodeDetailsContent.append("\t\t\"NodeIP\":\"" + value.trim() + "\",\n");	//Ignore the initial "["
						break;
					case 1:
						...
						try {
							if (value.contains("\"\"") || value.contains("NOT-SET")) {
								...
								if( m.matches()) {
									...
									Logger.error("Could not match LatLong " + value);
								}
							}
						} catch (Exception e) {
					...
					case 17:
						...
						nodeDetailsContent.append("\t\t\"NodeComments\":\"" + Utils.removeLastCharacterFromString(value) + "\"\n");	//The last line of node description doesn't end with comma
						break;
					default:
						break;
					}
