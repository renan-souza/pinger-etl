...
public class Utils {
	...
	private static String readGZipFile(String filePath, boolean saveFileSize, String metric, String year, String month, String day) {
		...
		try {
			...
			if (saveFileSize){
				openAppendContentAndClose(metric+","+year+","+month+","+day+","+Long.toString(everything.length()), C.filesSizeLog);
			}
			return everything;
		} catch (Exception e) {
			Logger.log("FILE NOT FOUND! -- Could not read the file " + f.getAbsolutePath(), e, "fileNotFound");
			if (saveFileSize){
				openAppendContentAndClose(metric+","+year+","+month+","+day+",", C.filesSizeLog);
			}
			return null;
		} finally {
			try {
	...
	public static String readGZipFileWithouSavingFileSize(String filePath) {
		return readGZipFile(filePath, false, null, null, null, null);
	}

	
	public static String readGZipFileAndSaveFileSize(String filePath, String metric, String year, String month, String day) {
		return readGZipFile(filePath, true, metric, year, month, day);
	}
	...
	public static void openAppendContentAndClose(String content, String filePath) {
		try {
			File fout = new File(filePath);
			if (!fout.exists()) {
				fout.createNewFile();
			...
			Logger.log("openAppendContentAndClose", e, "errors");
		}
	}
	...
	public static File[] getAllFilePathsInADir(String dir) {
		try {
			File fileDir = new File(dir);
			if (!fileDir.isDirectory()) {
				Logger.log("There is no such directory: " + dir, "errors");
				return null;
			}
			File files[] = fileDir.listFiles();
			return files;
	...
	@Deprecated
	public static String getFileNameBeginningHourly(String metric, String year, String month, String day) {
		if (fileNameBeginningHourly!=null) {
			return fileNameBeginningHourly;
		} else {
			fileNameBeginningHourly = year + "_" + month + "_" + metric + "_" +  day;
			return fileNameBeginningHourly;
		}
	}

	
	private static final String fileNameEnding = ".txt.gz";
	public static String getFileNameHourlyFtpData(String metric, String year, String month, String day) {
		String fileNameHourlyBeginningFtpData = metric + "-" + C.DEFAULT_PACKET_SIZE + "-by-node-"+ year + "-" + month + "-";
		return fileNameHourlyBeginningFtpData + day + fileNameEnding;
	}
